...
class SCHMD(object):
    ...
    @staticmethod
    def _parsetxt(txt, resp=None, enc=None,
                  history=None):  # {'cache':[],'fail':[]}):
    ...
    def _hackisotc211(txt):
        ...
        return txt.replace(bytes(SL[0], 'utf-8'),
                           bytes(SL[SLi], 'utf-8')) if SLi else txt

    def conditional(self, md=None):
        '''Bonus conditional extent checks'''
        dataset = False
        if md is not None: self.md = md
        for element in self.md.findall('gmd:hierarchyLevel/gmd:MD_ScopeCode',
                                       namespaces=NSX):
            if not element.get('codeListValue'):
                raise MetadataConditionalException(
                    'No Hierarchy Level Declared')
            elif element.get('codeListValue') == 'dataset':
                dataset = True

        if dataset:
            idin = self.md.find('gmd:identificationInfo', namespaces=NSX)
            for mddid in idin.iterfind('gmd:MD_DataIdentification',
                                       namespaces=NSX):
                if not isinstance(
                        mddid.find('gmd:topicCategory/gmd:MD_TopicCategoryCode',
                                   namespaces=NSX), _Element):
                    raise MetadataConditionalException(
                        'No Topic Category Declared')
                if not isinstance(mddid.find('gmd:extent', namespaces=NSX),
                                  _Element):
                    ...
                    extent = mddid.find(
                        ...
                        namespaces=NSX)
                    if not isinstance(
                            extent.find('gmd:EX_GeographicBoundingBox',
                                        namespaces=NSX), _Element) \
                            and not isinstance(
                        extent.find('gmd:EX_GeographicDescription',
                                    namespaces=NSX), _Element):
                        raise MetadataConditionalException(
                            'No Geographic Bounding Box or Geographic Description Declared')

        if not isinstance(self.md.find('gmd:language/gco:CharacterString',
                                       namespaces=NSX), _Element):
            raise MetadataConditionalException('No language/CS defined')

        if not isinstance(
                self.md.find('gmd:characterSet/gmd:MD_CharacterSetCode',
                             namespaces=NSX), _Element):
            raise MetadataConditionalException('No CharacterSetCode defined')

        mddid = self.md.find('gmd:identificationInfo/gmd:MD_DataIdentification',
                             namespaces=NSX)
        if not isinstance(
                mddid.find('gmd:language/gco:CharacterString', namespaces=NSX),
                _Element):
            raise MetadataConditionalException('No ID Info language/CS defined')

        if not isinstance(mddid.find('gmd:characterSet/gmd:MD_CharacterSetCode',
                                     namespaces=NSX), _Element):
            raise MetadataConditionalException(
                'No ID Info CharacterSetCode defined')
...
class Local(SCHMD):
    ...
    def schema(cls):
        ...
        sch_path = os.path.abspath(
            os.path.join(os.path.dirname(__file__), cls.SP, sch_pr.netloc,
                         sch_pr.path[1:], 'gmd', sch_name))
    ...
    def metadata(cls, md_name):
        ...
        md_path = os.path.abspath(
            os.path.join(os.path.dirname(__file__), cls.TP,
                         md_name or md_sample))
    ...
    def _copyschemas(cls, pr):
        ...
        os.chdir(
            os.path.abspath(os.path.join(os.path.dirname(__file__), cls.SP)))
        os.system('wget -r -np -R "index.html*" {}://{}{}'.format(pr.scheme,
                                                                  pr.netloc,
                                                                  pr.path))
...
class Remote(SCHMD):
    ...
    def metadata(cls, lid):
        ...
        try:
            ...
            if 'https://id.koordinates.com/login' in md_handle.url:
                raise InaccessibleMetadataException(
                    'Private layer {}.\n{}'.format(lid, xse))
            else:
    ...
    def _bextract(self, bst, ftx):
        ...
        for ft in bst.find_all(ftx['path']):
            ...
            ret[match.group(1)] += (
            (int(match.group(2)), ft.find(ftx['title']).text),)
        return ret
    ...
    def _xextract(self, xft, ftx):
        ...
        for ft in xft.findall(ftx['path'], namespaces=NSX):
            match = re.search('(layer|table)-(\d+)',
                              ft.find(ftx['name'], namespaces=NSX).text)
            ret[match.group(1)] += (
